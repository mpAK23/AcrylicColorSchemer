...
seriesSelect.addEventListener('change', (e) => {
    ...
    document.getElementById('opacityFilter').value = 'all';
    applyFilters();
});
...
function applyFilters() {
    ...
    const opacityFilter = document.getElementById('opacityFilter').value;
    ...
    const filtered = paints.filter(p => {
        ...
        const matchOpacity = opacityFilter === 'all' || p.opacity === opacityFilter;
        return matchSearch && matchOwned && matchOpacity;
    });
...
document.getElementById('opacityFilter').addEventListener('change', applyFilters);
...
function renderPaints(paintList) {
    ...
    paintList.forEach(paint => {
        ...
        item.innerHTML = `
            ...
            <div class="paint-pigment">${paint.pigment} &middot; <span style="text-transform: capitalize;">${paint.opacity || 'Unknown'}</span></div>
